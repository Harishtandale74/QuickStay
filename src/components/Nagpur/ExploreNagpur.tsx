...
// Import your data - make sure these exist
// import { NAGPUR_LANDMARKS, TRAVEL_TIPS, EMERGENCY_CONTACTS } from '../../data/nagpurData';

// Fallback data in case imports are not available
const TRAVEL_TIPS = [
  "Carry water bottle - Nagpur can get very hot",
  "Best time to visit is October to February",
  "Try local oranges - Nagpur is famous for them",
  "Auto rickshaws are the most convenient transport",
...
const ExploreNagpur: React.FC = () => {
  ...
  // Static data with proper fallbacks
  ...
  const shoppingPlaces: ShoppingPlace[] = useMemo(() => [
    {
      id: '1',
      name: 'Sitabuldi Main Road',
      type: 'Street',
  ...
  const transportModes: TransportMode[] = useMemo(() => [
    {
      mode: 'Auto Rickshaw',
      description: 'Most convenient for short distances within the city',
      fare: '₹15-20 per km',
  ...
  const categories = useMemo(() => {
    const uniqueCategories = Array.from(new Set(attractions.map(a => a.category)));
  ...
  const shareAttraction = useCallback(async (attraction: Attraction) => {
    try {
      if (navigator.share) {
        ...
        await navigator.clipboard.writeText(`Check out ${attraction.name} in Nagpur! ${attraction.description}`);
  ...
  useEffect(() => {
    const savedFavorites = localStorage.getItem('nagpur-favorites');
    if (savedFavorites) {
      try {
  ...
  useEffect(() => {
    localStorage.setItem('nagpur-favorites', JSON.stringify(favorites));
  }, [favorites]);
  ...
  const tabConfig = useMemo(() => [
    ...
    { 
      ...
      count: shoppingPlaces.length 
    },
    { 
      id: 'transport', 
      label: 'Transport', 
      icon: <Car className="h-4 w-4" />, 
      count: transportModes.length 
    },
    { 
      id: 'tips', 
      label: 'Travel Tips', 
      icon: <Info className="h-4 w-4" />, 
      count: TRAVEL_TIPS.length 
    },
  ], [filteredAttractions.length, restaurants.length, shoppingPlaces.length, transportModes.length]);
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <div className="flex-1 relative">
            ...
            <input
              ...
              aria-label="Search attractions and places"
            />
          </div>
          <div className="flex items-center space-x-2">
            ...
            <select
              value={selectedCategory}
              onChange={(e) => setSelectedCategory(e.target.value)}
              className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-colors"
      ...
      {/* Tabs */}
      <div className="border-b border-gray-200">
        <nav className="flex overflow-x-auto space-x-8 px-6" role="tablist">
          {tabConfig.map((tab) => (
            <button
              key={tab.id}
      ...
      <div className="p-6">
        {isLoading ? (
          ...
          <>
            {/* Attractions Tab */}
            {activeTab === 'attractions' && (
              <div className="space-y-6" role="tabpanel" id="attractions-panel">
                <div className="flex items-center justify-between">
                  <h4 className="text-xl font-bold text-gray-900">
                    Top Attractions in Nagpur ({filteredAttractions.length})
                  </h4>
                ...
                {filteredAttractions.length === 0 ? (
                  <ErrorMessage message="No attractions found matching your criteria" />
                ) : (
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    {filteredAttractions.map((attraction) => (
                      <div key={attraction.id} className="border border-gray-200 rounded-xl overflow-hidden hover:shadow-lg transition-all duration-300">
                        <div className="relative">
                          <OptimizedImage
                            ...
                            alt={attraction.name}
                            className="w-full h-48 object-cover"
                            fallbackSrc="https://images.pexels.com/photos/161401/fushimi-inari-taisha-shrine-kyoto-japan-161401.jpeg?auto=compress&cs=tinysrgb&w=400"
                          />
                          <div className="absolute top-4 left-4">
                            <span className="bg-orange-500 text-white px-3 py-1 rounded-full text-sm font-medium">
                              {attraction.category}
                          ...
                          <div className="absolute bottom-4 right-4 bg-white/95 backdrop-blur-sm px-3 py-1 rounded-lg flex items-center space-x-1">
                            ...
                            <span className="text-sm font-semibold">{attraction.rating}</span>
                            <span className="text-xs text-gray-600">({attraction.reviewCount.toLocaleString()})</span>
                          </div>
                        </div>
                        ...
                        <div className="p-6">
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <button className="flex items-center space-x-1 text-orange-600 hover:text-orange-700 font-medium text-sm transition-colors">
                              <span>View Details</span>
                              <ChevronRight className="h-4 w-4" />
                            </button>
                          </div>
            ...
            {/* Food Tab */}
            {activeTab === 'food' && (
              <div className="space-y-6" role="tabpanel" id="food-panel">
                <h4 className="text-xl font-bold text-gray-900">Best Food & Restaurants in Nagpur</h4>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {restaurants.map((restaurant) => (
                    <div key={restaurant.id} className="border border-gray-200 rounded-xl overflow-hidden hover:shadow-lg transition-shadow">
            ...
            {/* Shopping Tab */}
            {activeTab === 'shopping' && (
              <div className="space-y-6" role="tabpanel" id="shopping-panel">
                <h4 className="text-xl font-bold text-gray-900">Shopping Destinations in Nagpur</h4>
                <div className="space-y-4">
                  {shoppingPlaces.map((place) => (
                    <div key={place.id} className="border border-gray-200 rounded-xl p-6 hover:shadow-lg transition-shadow">
                      <div className="flex items-start space-x-4">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between mb-2">
                            <h5 className="text-xl font-bold text-gray-900 truncate">{place.name}</h5>
            ...
            {/* Transport Tab */}
            {activeTab === 'transport' && (
              <div className="space-y-6" role="tabpanel" id="transport-panel">
                <h4 className="text-xl font-bold text-gray-900">Getting Around Nagpur</h4>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {transportModes.map((transport, index) => (
                    <div key={index} className="border border-gray-200 rounded-xl p-6 hover:shadow-lg transition-shadow">
                      <div className="flex items-center space-x-3 mb-4">
                        <span className="text-3xl" aria-hidden="true">{transport.icon}</span>
                        <div>
                          <h5 className="text-lg font-bold text-gray-900">{transport.mode}</h5>
                          <p className="text-gray-600 text-sm">{transport.description}</p>
                        </div>
            ...
            {/* Tips Tab */}
            {activeTab === 'tips' && (
              <div className="space-y-6" role="tabpanel" id="tips-panel">
                <h4 className="text-xl font-bold text-gray-900">Essential Travel Tips & Information</h4>
                    ...
                    {Object.entries(EMERGENCY_CONTACTS).map(([service, number]) => (
                      <div key={service} className="flex items-center justify-between p-3 bg-white rounded-lg">
                        ...
                        <a
                          href={`tel:${number}`}
                          className="text-red-900 hover:text-red-700 font-bold flex items-center space-x-1 bg-red-100 px-3 py-1 rounded-lg hover:bg-red-200 transition-colors"
                          aria-label={`Call ${service} at ${number}`}
                        >
                          <Phone className="h-4 w-4" />
                          <span>{number}</span>
                        </a>
