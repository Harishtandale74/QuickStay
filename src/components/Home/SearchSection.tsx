...
const SearchSection: React.FC<SearchSectionProps> = ({ deviceType }) => {
    ...
    switch (deviceType) {
      case 'mobile':
        return {
          ...
          buttonSize: 'px-8 py-3 text-base',
  ...
  return (
    <section className={`bg-white ${config.padding} -mt-8 relative z-10`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Main Search Card */}
        <div className={`bg-white rounded-3xl shadow-2xl ${config.cardPadding} border border-gray-100 relative overflow-hidden`}>
          {/* Background Pattern */}
          ...
          <div className="relative">
            ...
            <div className="text-center mb-8">
              ...
              <h2 className={`${config.titleSize} font-bold text-gray-900 mb-2`}>Find Your Perfect Stay</h2>
            ...
            <form onSubmit={handleSearch} className="space-y-6">
              ...
              <div className={`grid ${deviceType === 'mobile' ? 'grid-cols-1' : 'grid-cols-1 md:grid-cols-3'} gap-4 md:gap-6`}>
                {/* Check-in */}
                <div className="space-y-2">
                  <label className="block text-sm font-medium text-gray-700">
                    Check-in
              ...
              <div className="flex justify-center">
                <button
                  type="submit"
                  className={`group bg-gradient-to-r from-primary-600 to-secondary-600 hover:from-primary-700 hover:to-secondary-700 text-white ${config.buttonSize} rounded-xl font-semibold transition-all duration-300 transform hover:scale-105 flex items-center space-x-3 shadow-lg hover:shadow-xl`}
                >
        ...
        {config.showQuickSearch && (
        <div className="mt-6 md:mt-8">
          <div className="text-center mb-6">
            <h3 className={`${deviceType === 'mobile' ? 'text-base' : 'text-lg'} font-semibold text-gray-900 mb-2`}>Quick Search</h3>
            <p className="text-gray-600">Popular searches powered by AI insights</p>
          </div>
          
          <div className={`grid ${config.gridCols} gap-3 md:gap-4`}>
            {quickSearches.map((item, index) => (
              <button
                ...
                className={`group bg-white hover:bg-gradient-to-r hover:from-primary-50 hover:to-secondary-50 border border-gray-200 hover:border-primary-300 rounded-xl ${deviceType === 'mobile' ? 'p-3' : 'p-4'} transition-all duration-300 transform hover:scale-105 hover:shadow-lg`}
              >
                <div className="flex items-center space-x-3">
                  <div className="text-primary-600 group-hover:text-primary-700 transition-colors">
                    {item.icon}
                  </div>
                  <span className={`font-medium text-gray-900 group-hover:text-primary-900 transition-colors ${deviceType === 'mobile' ? 'text-sm' : ''}`}>
                    {item.label}
                  </span>
                </div>
              </button>
        ...
        <div className="mt-6 p-4 bg-gradient-to-r from-purple-50 to-pink-50 rounded-2xl border border-purple-200">
          ...
          <div className={`grid ${deviceType === 'mobile' ? 'grid-cols-1' : 'grid-cols-1 md:grid-cols-2'} gap-3`}>
